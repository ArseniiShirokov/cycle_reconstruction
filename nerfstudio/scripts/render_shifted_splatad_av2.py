...
class ShiftedDatasetRender(BaseRender):
    ...
    def main(self):
        ...
        for split in self.pose_source.split("+"):
            ...
            lidar_dataloader = datamanager.fixed_indices_eval_lidar_dataloader
            ...
            if self.render_point_clouds:
                with Progress(
                    ...
                    with torch.no_grad():
                        ...
                        if hasattr(datamanager, "eval_lidar_dataset") and getattr(datamanager, "eval_lidar_dataset").metadata is not None:
                            sensor_idx_to_name = datamanager.eval_lidar_dataset.metadata.get("sensor_idx_to_name", None)
                        
                        # Get actual lidar filenames from dataset - MUST use exact filenames from dataset
                        lidar_filenames = None
                        if hasattr(datamanager, "eval_lidar_dataset"):
                            if hasattr(datamanager.eval_lidar_dataset, "filenames"):
                                lidar_filenames = datamanager.eval_lidar_dataset.filenames
                            elif hasattr(datamanager.eval_lidar_dataset, "metadata") and datamanager.eval_lidar_dataset.metadata:
                                lidar_filenames = datamanager.eval_lidar_dataset.metadata.get("pc_filenames", None)
                        ...
                        for lidar_idx, (lidar, batch) in enumerate(
                            ...
                            outputs = pipeline.model.get_lidar_outputs(lidar)

                            gt_lidar = batch["raster_pts"]
                            gt_valid = gt_lidar[..., 2].flatten() > 0
                            outputs["ray_drop"] = outputs["ray_drop_logits"].flatten() * gt_valid - (~gt_valid) * 10_000

                            median_depth = outputs["median_depth"].flatten()
                            ...
                            gt_point_cloud = batch["lidar"][batch["lidar_pts_did_return"].squeeze(), :3]
                            median_point_cloud = (
                                median_depth.view(-1, 1) * directions
                                + (batch["linear_velocities_local"].squeeze() * gt_lidar[..., 3].view(-1, 1))
                            )
                            mask = (outputs["ray_drop"].sigmoid() <= 0.5).view(-1) * gt_valid.view(-1)
                            median_point_cloud = median_point_cloud[mask]
                            intensity = outputs["intensity"].flatten()[mask]
                            # Scale to AV2 convention [0..255] (parser divides by 255 later)
                            intensity_uint8 = (intensity.clamp(0, 1) * 255.0).round().to(torch.uint8)

                            # Determine lidar name (up/down)
                            lidar_name = None
                            ...
                            # Assign laser_number by nearest elevation channel
                            from nerfstudio.data.utils.lidar_elevation_mappings import ARGOVERSE2_VELODYNE_VLP32C_ELEVATION_MAPPING as EL_MAP
                            channel_order = sorted(EL_MAP.keys())
                            channel_elev = torch.tensor([EL_MAP[k] for k in channel_order], dtype=torch.float32, device=median_point_cloud.device)
                            elev_deg = gt_lidar[..., 1].flatten()[mask]  # already degrees
                            diffs = torch.abs(elev_deg[:, None] - channel_elev[None, :])
                            nearest = torch.argmin(diffs, dim=1).to(torch.int32)
                            if lidar_name == "lidar_down":
                                laser_number = nearest + 32
                            else:
                                laser_number = nearest

                            # Get time from outputs (centered if compensate_rs_lidar is enabled)
                            time_output = outputs["time"].flatten()[mask]
                            
                            # Convert time to ns (will add back adjustment later when combining)
                            offset_ns = (time_output * 1e9).to(torch.int64)

                            # Use lidar frame coordinates as generated
                            xyz = median_point_cloud

                            # Sweep timestamp key
                            sweep_time = None
                            sweep_time = float(lidar.times[0].item())

                            # Store actual file path from dataloader - MUST use exact filename from dataset
                            if sweep_time not in sweep_buffer:
                                sweep_buffer[sweep_time] = {}
                                # For Argoverse2 with output_lidars_separately=True, up and down share same timestamp/file
                                # Map lidar_idx to file index (handle case where 2N lidars map to N files)
                                ...
                                if 0 <= file_idx < num_files:
                                    ...
                                    CONSOLE.print(f"[yellow]Warning: Could not map lidar_idx {lidar_idx} to file index. Skipping.[/yellow]")
                                    continue
